...
#
a$Rate[is.na(a$Rate)]<-0
a$황사<-as.character(a$황사)
a$황사[a$황사=="매우나쁨"]<-"나쁨"
unique(a$황사)
a$황사<-as.factor(a$황사)
str(a)
...
windows()
ggplot(a11, aes(x=한파,y=Rate,fill=한파))+
  geom_boxplot(size=1, aes(group=한파))+
  geom_hline(yintercept = (median(subset(a11, 한파=="1")$Rate)+median(subset(a11,한파=="0")$Rate))/2)
mean(subset(a11,))
str(subset(a11, 한파=="1"))
sum(all$한파 == 1)
mean(14.33333,15.7975)

ggplot(a11, aes(x=대설,y=Rate,fill=대설))+
  geom_boxplot(size=1, aes(group=대설))+
  geom_hline(yintercept = 15.4) 

ggplot(a11, aes(x=강풍,y=Rate,fill=강풍))+
  geom_boxplot(size=1, aes(group=강풍))+
  geom_hline(yintercept = 14.8) 

ggplot(a11, aes(x=건조,y=Rate,fill=건조))+
  geom_boxplot(size=1, aes(group=건조))+
  geom_hline(yintercept = 14.8) 

ggplot(a11, aes(x=호우,y=Rate,fill=호우))+
  geom_boxplot(size=1, aes(group=호우))+
  geom_hline(yintercept = 14.7) 

ggplot(a11, aes(x=폭염,y=Rate,fill=폭염))+
  geom_boxplot(size=1, aes(group=폭염))+
  geom_hline(yintercept = 13.8) 

ggplot(a11, aes(x=황사,y=Rate,group=황사,fill=황사))+
  geom_boxplot()
...
summary(lm(Rate~.-Date-요일-호우-강풍-대설-건조-한파-폭염-황사,data=a))
...
ggarrange(we6,ev6, 
          ...
          ncol = 2, nrow = 1)
#시각화한 결과를 보면 주말이 역시 평일보다 시청률이 전반적으로 높은 것을 알 수 있음.
